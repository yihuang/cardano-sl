...
send pm epochSlots diffusion idx outputs = do
    ...
    let plainAddresses = map
            (flip makePubKeyAddress curPk . IsBootstrapEraAddr)
            [False, True]
    let
        (hdAddresses, hdSecrets) = unzip $ map
            (\ibea ->
                fromMaybe (error "send: pass mismatch") $ deriveFirstHDAddress
    ...
    etx <- withSafeSigners allSecrets (pure emptyPassphrase) $ \signers ->
        runExceptT @AuxxException $ do
            let addrSig = HM.fromList $ zip allAddresses signers
            let getSigner addr = HM.lookup addr addrSig
            -- BE CAREFUL: We create remain address using our pk, wallet doesn't
            -- show such addresses
            (txAux, _) <- lift $ prepareMTx pm
                                            epochSlots
                                            getSigner
                                            mempty
                                            def
  ...
  where
    ...
    takeSecret
        | idx == -1 = do
            ...
            pure $ maybe (error "Unknown wallet address")
                         (^. wusRootKey)
                         (_userSecret ^. usWallet)

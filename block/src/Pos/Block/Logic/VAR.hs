...
verifyBlocksPrefix logTrace blocks = runExceptT $ do
    ...
    _ <- withExceptT (VerifyBlocksError . pretty) $
        ExceptT $ sscVerifyBlocks (named logTrace) (map toSscBlock blocks)
    TxpGlobalSettings {..} <- view (lensOf @TxpGlobalSettings)
    txUndo <- withExceptT (VerifyBlocksError . pretty) $
        ExceptT $ tgsVerifyBlocks (named logTrace) dataMustBeKnown $ map toTxpBlock blocks
    pskUndo <- withExceptT VerifyBlocksError $ dlgVerifyBlocks blocks
    (pModifier, usUndos) <- withExceptT (VerifyBlocksError . pretty) $
        ExceptT $ usVerifyBlocks logTrace dataMustBeKnown (map toUpdateBlock blocks)
...
verifyAndApplyBlocks logTrace rollback blocks = runExceptT $ do
    tip <- lift GS.getTip
    ...
    lift $ normalizeMempool (named logTrace)
    pure hh
  where
    ...
    applyAMAP e (OldestFirst (block:xs)) nothingApplied =
        lift (verifyBlocksPrefix logTrace (one block)) >>= \case
            Left (ApplyBlocksVerifyFailure -> e') ->
                applyAMAP e' (OldestFirst []) nothingApplied
            Right (OldestFirst (undo :| []), pModifier) -> do
                lift $ applyBlocksUnsafe logTrace (ShouldCallBListener True) (one (block, undo)) (Just pModifier)
    ...
    failWithRollback e toRollback = do
        lift $ logDebug (named logTrace) "verifyAndapply failed, rolling back"
        lift $ mapM_ (rollbackBlocks (named logTrace)) toRollback
        throwError e
    ...
    rollingVerifyAndApply blunds (prefix, suffix) = do
        ...
        when (isLeft prefixHead) $ do
            ...
            lift $ logDebug (named logTrace) $ "Rolling: Calculating LRC if needed for epoch "
                       <> pretty epochIndex
            lift $ lrcSingleShot logTrace epochIndex
        lift $ logDebug (named logTrace) "Rolling: verifying"
        lift (verifyBlocksPrefix logTrace prefix) >>= \case
            Left (ApplyBlocksVerifyFailure -> failure)
                | rollback  -> failWithRollback failure blunds
                | otherwise -> do
                      lift $ logDebug (named logTrace) "Rolling: Applying AMAP"
                      applyAMAP failure
            ...
            Right (undos, pModifier) -> do
                ...
                lift $ logDebug (named logTrace) "Rolling: Verification done, applying unsafe block"
                lift $ applyBlocksUnsafe logTrace (ShouldCallBListener True) newBlunds (Just pModifier)
                case getOldestFirst suffix of
                    [] -> lift GS.getTip
                    (genesis:xs) -> do
                        lift $ logDebug (named logTrace) "Rolling: Applying done, next portion"
...
applyBlocks logTrace calculateLrc pModifier blunds = do
    when (isLeft prefixHead && calculateLrc) $
        ...
        lrcSingleShot logTrace (prefixHead ^. epochIndexL)
    applyBlocksUnsafe logTrace (ShouldCallBListener True) prefix pModifier
    case getOldestFirst suffix of
        []           -> pass
        (genesis:xs) -> applyBlocks logTrace calculateLrc pModifier (OldestFirst (genesis:|xs))
  where
...
rollbackBlocks
    ...
    => Trace m LogItem -> NewestFirst NE Blund -> m ()
rollbackBlocks logTrace blunds = do
    tip <- GS.getTip
    ...
    rollbackBlocksUnsafe logTrace (BypassSecurityCheck False) (ShouldCallBListener True) blunds
...
applyWithRollback logTrace toRollback toApply = runExceptT $ do
    tip <- lift GS.getTip
    when (tip /= newestToRollback) $
        throwError $ ApplyBlocksTipMismatch "applyWithRollback/rollback" tip newestToRollback

    let doRollback = rollbackBlocksUnsafe (named logTrace)
  ...
  where
    ...
    applyBack :: m ()
    applyBack = applyBlocks logTrace False Nothing reApply
    ...
    onGoodRollback =
        verifyAndApplyBlocks logTrace True toApply >>= \case

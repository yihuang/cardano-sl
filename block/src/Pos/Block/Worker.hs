...
import           Pos.Core (BlockCount, BlockVersionData (..), ChainDifficulty,
                     ...
                     slotIdF, slotIdSucc, unflattenSlotId)
...
informerWorker k _ =
    onNewSlot epochSlots defaultOnNewSlotParams
        $ \slotId -> recoveryCommGuard k "onNewSlot worker, informerWorker" $ do
...
recoveryTriggerWorker pm k diffusion = do
    ...
    repeatOnInterval $ do
        doTrigger <- needTriggerRecovery
            <$> getSyncStatus epochSlots (kSlotSecurityParam k)
        when doTrigger $ do

...
createMTx pm epochSlots pendingTx groupInputs utxo hdwSigners outputs addrData
    = createGenericTx pm
                      ...
                      addrData
  where
    getSigner address = note (SafeSignerNotFound address) $ hdwSigners address
...
createTx pm epochSlots pendingTx utxo ss outputs addrData =
    createGenericTxSingle pm
                          epochSlots
                          pendingTx
                          (\i o -> Right $ makePubKeyTx pm ss i o)

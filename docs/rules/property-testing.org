...
* Approach
  ...
  Sensible approach might be to use the inductive wallet generator to build a (valid) chain, then to try extending it in various (invalid) ways, and check that the result is still valid.

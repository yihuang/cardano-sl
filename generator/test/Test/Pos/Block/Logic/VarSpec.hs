...
import           Pos.Core (GenesisData (..), HasConfiguration,
                     ProtocolConstants (..), genesisData, headerHash)
...
verifyBlocksPrefixSpec :: HasStaticConfigurations => Spec
...
verifyEmptyMainBlock = do
    ...
    whenLeftM
            (lift $ verifyBlocksPrefix dummyProtocolMagic
                                       dummyProtocolConstants
                                       (one emptyBlock)
            )
        $ stopProperty
...
genSuccessWithForks
    :: forall g m . (RandomGen g, Monad m) => BlockEventGenT g m ()
genSuccessWithForks = do
  ...
  where
    ...
    generateFork basePath rollbackFork = do
        let forkLen    = length rollbackFork
            ...
            then whenJust (nonEmptyNewestFirst rollbackFork)
                ...
                needRollback <-
                    ...
                    -- forkLen=blkSecurityParam => needRollback 100%
                                byChance
                    (realToFrac $ forkLen Ratio.% fromIntegral dummyK)
                if needRollback
                    then do
                        retreat <- getRandomR (1, forkLen)
                        whenJust (nonEmptyNewestFirst rollbackFork)
                            $ \rollbackFork' -> do
                            -- forkLen > 0, therefore retreat > 0
                                  let
                                      (over _NewestFirst NE.fromList -> before, after)
                                          = splitAtNewestFirst retreat
                                                               rollbackFork'
                        ...
                        relPaths <-
                            OldestFirst
                                <$> replicateM advance generateRelativePath1
                        whenJust (nonEmptyOldestFirst relPaths) $ \relPaths' ->
                            do
                                let
                                    curPath =
                                        maybe basePath NE.head $ nonEmpty
                                            (getNewestFirst rollbackFork)
...
genSingleFork
    :: forall g m . (RandomGen g, Monad m) => ForkDepth -> BlockEventGenT g m ()
genSingleFork fd = do
    ...
    let
        ...
        commonPrefix =
            pathSequence mempty $ OldestFirst . nonEmptyCuz "i > 0" $ replicate
                i
                "B"
        originalChain =
            pathSequence mempty $ OldestFirst . nonEmptyCuz "q > 0" $ replicate
                q
                "B"
        rollbackChain =
            toNewestFirst
                . pathSequence (stimes i "B")
                $ OldestFirst
                . nonEmptyCuz "d > 0"
                $ replicate d "B"
        forkChain =
            pathSequence (NE.last $ getOldestFirst commonPrefix)
                $ OldestFirst
                . nonEmptyCuz "dj > 0"
                $ replicate dj "C"
    ...
    if expectSuccess
        then do
            ...
            emitBlockRollback
                (BlockRollbackFailure BlkRbSecurityLimitExceeded)
                rollbackChain

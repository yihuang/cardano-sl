...
logicFull pm pc ourStakeholderId securityParams jsonLogTx =
    let
        ...
        postTx       = KeyVal
            ...
            , handleInv  = \(Tagged txId) ->
                not
                    .   HM.member txId
                    .   _mpLocalTxs
                    <$> withTxpLocalData getMemPool
            , handleReq  = \(Tagged txId) ->
                fmap TxMsgContents
                    .   HM.lookup txId
                    .   _mpLocalTxs
                    <$> withTxpLocalData getMemPool
            , handleData = \(TxMsgContents txAux) ->
                Txp.handleTxDo pm epochSlots jsonLogTx txAux
            }
        ...
        postUpdate = KeyVal
            ...
            where tag = tagWith (Proxy :: Proxy (UpdateProposal, [UpdateVote]))

        postVote = KeyVal
            { toKey      = \UnsafeUpdateVote {..} ->
                pure $ tag (uvProposalId, uvKey, uvDecision)
            , handleInv  = \(Tagged (id, pk, dec)) ->
                Update.isVoteNeeded id pk dec
            , handleReq  = \(Tagged (id, pk, dec)) ->
                Update.getLocalVote id pk dec
            , handleData = Update.handleVote pm k
            }
            where tag = tagWith (Proxy :: Proxy UpdateVote)
        ...
        postSscCommon sscTag contentsToKey toContents processData = KeyVal
            ...
            , handleReq  = \(Tagged addr) ->
                toContents addr . view ldModifier <$> sscRunLocalQuery ask
            , handleData = \dat -> do

...
-- context has.
cloneLrcContext ::
       (MonadIO m, MonadThrow m) => Trace m (Severity, Text) -> LrcContext -> m LrcContext
cloneLrcContext logTrace LrcContext {..} = do
    readTVarIO lcLrcSync >>= \case
        lsd@LrcSyncData {..}
            | lrcNotRunning -> LrcContext <$> newTVarIO lsd
            | otherwise ->
                -- FIXME can we get rid of this? Or at least do better?
                -- Why is it weird? Is it really fatal? What will happen in
                -- this case?

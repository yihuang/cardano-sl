...
module Pos.Network.Topology.Yaml
       (
         Topology(..)
...
import           Pos.Network.Topology.DnsDomains (DnsDomains (..),
                     NodeAddr (..))
import           Pos.Network.Topology.Types (Fallbacks, NodeName (..), Valency)
import           Pos.Util.Util (aesonError, toAesonError)

-- | Description of the network topology in a Yaml file
--
-- This differs from 'Pos.Network.Topology.Types.Topology' because for static nodes this

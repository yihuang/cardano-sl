...
import           Pos.Core (BlockCount, BlockVersionData (..), EpochIndex,
                     HasGenesisData, ProtocolConstants, SlotId (..),
                     StakeholderId, VssCertificate, epochIndexL, kEpochSlots,
                     mkVssCertificatesMapSingleton, pcBlkSecurityParam,
                     pcEpochSlots)
...
sscGetLocalPayloadQ :: BlockCount -> SlotId -> SscLocalQuery SscPayload
...
sscNormalize pm pc = do
    ...
    launchNamedPureLog atomically $
        ...
        sscNormalizeU pm pc (tipEpoch, richmenData) bvd gs
  where
...
sscNormalizeU
    ...
    -> SscGlobalState
    -> SscLocalUpdate ()
sscNormalizeU pm pc (epoch, stake) bvd gs = do
    ...
    newModifier <-
        ...
        execTossT mempty $ normalizeToss pm pc epoch oldModifier
...
sscIsDataUseful k tag id =
    ifM
        (maybe False (isGoodSlotForTag k tag . siSlot) <$> getCurrentSlot (kEpochSlots k))
...
sscProcessCommitment pm pc comm =
    sscProcessData pm pc CommitmentMsg
...
sscProcessOpening
    ...
    -> Opening
    -> m (Either SscVerifyError ())
sscProcessOpening pm pc id opening = sscProcessData pm pc OpeningMsg
...
sscProcessShares
    ...
    -> InnerSharesMap
    -> m (Either SscVerifyError ())
sscProcessShares pm pc id shares =
    sscProcessData pm pc SharesMsg $ SharesPayload (HM.fromList [(id, shares)]) mempty
...
sscProcessCertificate
    ...
    -> VssCertificate
    -> m (Either SscVerifyError ())
sscProcessCertificate pm pc cert = sscProcessData pm pc VssCertificateMsg
...
sscProcessData
    ...
    -> SscPayload
    -> m (Either SscVerifyError ())
sscProcessData pm pc tag payload =
    runExceptT $ do
        getCurrentSlot (pcEpochSlots pc) >>= checkSlot
        ld <- sscRunLocalQuery ask
        bvd <- gsAdoptedBVData
        ...
        lift (tryGetSscRichmen epoch) >>= \case
            ...
            Just richmen -> do
                ...
                ExceptT $
                    sscRunLocalSTM $
                    executeMonadBaseRandom seed $
                    sscProcessDataDo pm pc (epoch, richmen) bvd gs payload
  where
    checkSlot Nothing = throwError CurrentSlotUnknown
    checkSlot (Just si@SlotId {..})
        | isGoodSlotForTag (pcBlkSecurityParam pc) tag siSlot = pass
...
sscProcessDataDo
    ...
    -> SscPayload
    -> m (Either SscVerifyError ())
sscProcessDataDo pm pc richmenData bvd gs payload =
    runExceptT $ do
        ...
        oldTM <-
            if | not exhausted -> use ldModifier
               | otherwise ->
                   ...
                   execTossT mempty . refreshToss pm pc givenEpoch =<<
                   use ldModifier
        newTM <-
            ExceptT $
            evalPureTossWithLogger gs $
            ...
            runExceptT $
            execTossT oldTM $
            verifyAndApplySscPayload pm pc (Left storedEpoch) payload
        ldModifier .= newTM

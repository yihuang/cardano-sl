...
verifyAndApplyMultiRichmen logTrace onlyCerts env =
    tossToVerifier logTrace . hoist (supplyPureTossEnv env) . mapM_ verifyAndApplyDo
  where
...
sscRollbackBlocks logTrace blocks =
    sscRunGlobalUpdate logTrace $ do
        sscRollbackU blocks

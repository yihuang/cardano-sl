...
recordTxpMetrics
    ...
    -> Metrics.Store
    -> TVar MemPool
    -> IO (StateLockMetrics MemPoolModifyReason)
recordTxpMetrics logTrace ekgStore memPoolVar = do
    ...
    pure StateLockMetrics
        ...
        , slmAcquire = \reason timeWaited -> do
              ...
              logDebug logTrace $
                  sformat ("MemPool metrics acquire: "%shown%" wait time was "%shown)
                          reason
                          timeWaited
        ...
        , slmRelease = \reason timeWaited timeElapsed memAllocated -> do
              ...
              logDebug logTrace $
                  sformat ("MemPool metrics release: "%shown%" modify time was "%shown%" size is "%shown)
                          reason
                          timeElapsed
                          newMemPoolSize

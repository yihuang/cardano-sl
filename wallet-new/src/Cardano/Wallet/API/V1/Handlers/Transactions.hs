...
handlers :: ActiveWalletLayer Production -> ServerT Transactions.API Handler
...
newTransaction :: ActiveWalletLayer Production
               -> Payment
...
getTransactionsHistory :: PassiveWalletLayer Production
                       -> Maybe WalletId
                       -> Maybe AccountIndex
                       -> Maybe (V1 Address)
                       -> RequestParams
...
getTransactionsHistory pw mwalletId mAccIdx mAddr requestParams fops sops =
    liftIO $ runProduction $ respondWith requestParams fops sops (f <$> txs)
        where
            txs = WalletLayer.getTransactions pw mwalletId mAccIdx mAddr requestParams fops sops
            f (Left err) = error $ show err
            f (Right xs) = IxSet.fromList xs

-- | Computes the fees generated by this payment, without actually sending
-- the transaction to the network.
estimateFees :: ActiveWalletLayer Production
             -> Payment
...
estimateFees aw payment@Payment{..} = do
    ...
    res <- liftIO $ runProduction $ (WalletLayer.estimateFees aw) spendingPassword
                                                                  (toInputGrouping pmtGroupingPolicy)
                                                                  SenderPaysFee
                                                                  payment
    case res of

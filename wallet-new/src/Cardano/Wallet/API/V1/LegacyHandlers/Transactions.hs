...
newSignedTransaction submitTx (SignedTransaction encodedExtPubKey txAsHex signatureAsHex) = do
    ...
    txRaw <- case B16.decode txAsHex of
        Left problem ->
            throwM (UnknownError $ "Invalid transaction, it's not Base16 format: " <> toText problem)
        Right txRaw -> return txRaw

    tx <- case decodeFull' txRaw of
        Left problem ->
            throwM (UnknownError $ "Invalid transaction, unable to deserialize: " <> toText problem)
        Right (tx :: Tx) -> return tx

    signature <- case B16.decode signatureAsHex of
        Left problem ->
            throwM (UnknownError $ "Invalid signature, it's not Base16 format: " <> toText problem)
        Right signatureItself ->
            case xsignature signatureItself of
                Left problem ->
                    throwM (UnknownError $ "Invalid signature: " <> toText problem)

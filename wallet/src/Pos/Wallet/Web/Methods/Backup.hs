...
importWalletJSON :: MonadWalletBackup ctx m => GenesisHash -> CFilePath -> m CWallet
importWalletJSON genesisHash (CFilePath (toString -> fp)) = do
    contents <- liftIO $ BSL.readFile fp
    TotalBackup wBackup <- either parseErr pure $ A.eitherDecode contents
    restoreWalletFromBackup genesisHash wBackup
  where
